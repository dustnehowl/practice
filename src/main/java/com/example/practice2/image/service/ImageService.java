...
import com.example.practice2.image.repository.PostImageRepository;
...
public class ImageService {
    ...
    @Transactional
    public PostImage uploadPostImageS3(PostRequest postRequest) {
        Member uploader = findMemberById(postRequest.getUploaderId());
        MultipartFile file = postRequest.getImage();
        PostImage postImage = uploadPostFileToS3(file, uploader);
        postImageRepository.save(postImage);
        return postImage;
    }
    private PostImage uploadPostFileToS3(MultipartFile file, Member uploader){
        try{
            String originalName = file.getOriginalFilename();
            String storeFileName = createStoreFileName(originalName);
